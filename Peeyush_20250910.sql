...
-- 1. Write an SQL query to calculate the total revenue generated by each city. 
-- Assume a bookings table with columns: city, room_price, and number_of_nights.
...
-- 2. Write an SQL query to find customers who made more than 3 bookings in the last year. 
-- Use a bookings table with columns: customer_id and booking_date.
...
-- 3. Write an SQL query to calculate the cancellation rate for each property. 
-- Use a bookings table with columns: property_id, booking_id, and booking_status (confirmed/cancelled).

WITH set_properties AS (
  SELECT property_id, COUNT(*) AS confirmed
  FROM bookings
  WHERE booking_status = 'confirmed'
  GROUP BY property_id
), total_properties AS (
  SELECT property_id, COUNT(*) AS cancelled
  FROM bookings
  WHERE booking_status = 'cancelled'
  GROUP BY property_id
)
SELECT property_id, (CAST(confirmed AS FLOAT) / CAST(cancelled AS FLOAT)) AS rate  
FROM total_properties NATURAL JOIN set_properties;

-- 4. Write an SQL query to find the month with the highest revenue for each city. 
-- Assume a bookings table with columns: city, revenue, and booking_date.

WITH month_revenue AS (
  SELECT city, SUBSTRING(booking_date, 6, 2) AS month, SUM(revenue) AS month_revenue
  FROM bookings
  GROUP BY city, month
)
SELECT city, month, month_revenue
FROM month_revenue
GROUP BY city
HAVING month_revenue = MAX(month_revenue);

-- 5. Write an SQL query to find properties with occupancy rates greater than 90%. 
-- Assume a properties table with columns: property_id, total_rooms, and occupied_rooms.

-- There is no total_rooms and occupied_rooms in the dataset.
